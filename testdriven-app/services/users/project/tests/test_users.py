...
class TestUserService(BaseTestCase):
    ...
    def test_singel_user(self):
        """Ensure get single user behaves correctly"""
        user = add_user(username='alex', email='alex@test.com')
        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('alex', data['data']['username'])
            self.assertIn('alex@test.com', data['data']['email'])
            self.assertIn('success', data['status'])

    def test_singel_user_no_id(self):
    ...
    def test_singel_user_incorect_id(self):
